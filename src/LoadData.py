...
class LoadData(object):
    ...
    def __init__(self, path, dataset, loss_type):
        ...
        self.trainfile = self.path + "train_3358.txt"
        self.testfile = self.path + "test_3358.txt"
        # self.validationfile = self.path + "test_criteo.txt"
    ...
    def construct_dataset(self, X_, Y_):
        ...
        # print('-------------------------------')
        # print(indexs[0:20])
        Data_Dic['Y'] = [Y_[i] for i in range(len(Y_))]
        Data_Dic['X'] = [X_[i] for i in range(len(X_))]
        return Data_Dic
