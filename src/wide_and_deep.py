...
#################### Arguments ####################
def parse_args():
    parser = argparse.ArgumentParser(description="Run Wide-and-Deep.")
    ...
    parser.add_argument('--keep_prob', nargs='?', default='[.9, .9, .9, .9]',
...
class NeuralFM(BaseEstimator, TransformerMixin):
    ...
    def _init_graph(self):
        ...
        with self.graph.as_default():  # , tf.device('/cpu:0'):
            ...
            nonzero_weights = tf.nn.embedding_lookup(self.weights['linear_weights'], self.train_features) # 这个就是一阶偏置，ΣWiXi, None * M * 1
            self.lin_out = tf.reduce_sum(nonzero_weights, 1) # None * 1
            self.lin_out = tf.reshape(self.lin_out, [-1, 1]) # 降维后 None * 1
            nonzero_weights = tf.reshape(nonzero_weights, [-1, self.field_cnt]) # ((None *M)/f)  * f
            ...
            self.out = self.lin_out + self.weights['prediction_bias']
