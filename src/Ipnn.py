...
class NeuralFM(BaseEstimator, TransformerMixin):
    def __init__(self, field_cnt, features_M, hidden_factor, layers, loss_type, pretrain_flag, epoch, batch_size,
        ...
        self.nn_input_dim = self.field_cnt * self.hidden_factor + self.num_pairs  # fk+k
    ...
    def _init_graph(self):
        ...
        with self.graph.as_default():  # , tf.device('/cpu:0'):
            ...
            self.dropout_keep = tf.compat.v1.placeholder(tf.float32, shape=[None])
            ...
            if self.optimizer_type == 'adam':
                ...
                self.optimizer = tf.compat.v1.train.GradientDescentOptimizer(learning_rate=self.learning_rate).minimize(self.loss)
    ...
    def _initialize_weights(self):
        ...
        if num_layer > 0:
            ...
            maxval = np.sqrt(6. / np.sum([self.nn_input_dim, self.layers[0]]))
            minval = -maxval
            all_weights['layer_0'] = tf.Variable(tf.random.uniform([self.nn_input_dim, self.layers[0]], minval=minval,
